...
const StatusHeatmapContext =
    createContext<StatusHeatmapContextType | null>(null);
...
export type StatusHeatmapBlockProps =
...
export const StatusHeatmapBlock = forwardRef<
    SVGRectElement,
    StatusHeatmapBlockProps
>(({ activity, dayIndex, className, style: styleProp, ...props }, ref) => {
    const {
        blockSize,
        blockWidth,
        blockMargin,
                ...
                {showDateLabels && (
                    <g className={cn("fill-current font-mono text-xs", labelTextClass)}>
                        {dates.map((date, dayIndex) => {
                            ...
                            const xPosition = (blockWidth + blockMargin) * dayIndex + blockWidth / 2;
...
export const StatusHeatmapNormalDays = ({
    ...
    return (
        <div className={cn("text-muted-foreground", className)} {...props}>
            {labels.normalDays?.replace("{{count}}", String(normalDays)) ?? `${normalDays} days normal`}
        </div>
    );
};

...
export const WeekdayHeatmap = ({
    ...
    const width =
        ...
        (hasExtraColumn
            ? blockSize / 2 + (blockWidth + blockMargin) * 2
            : 0) -
        blockMargin +
        labelWidth;
...
export const WeekdayHeatmapBlock = ({
    ...
    const rowIndex = isExtraRow
        ? 7
        : (activity.weekday - weekStart + 7) % 7;
...
export const WeekdayHeatmapBody = ({
    ...
    const extraColumnMap = useMemo(() => {
        if (!extraColumn) return null;
        const map = new Map<number, { weekday: number; value: number; level: number }>();
                ...
                children ? (
                    ...
                    >
                        <rect
                            ...
                            ry={blockRadius}
                            width={blockWidth}
                            style={{ fill: getLevelFill(level, levels, isNormalized, false, colors) }}
                        />
                    </svg>
                )
            )}

...
export const CalendarHeatmapBody = ({
  ...
  return (
    <div
      ...
      {...props}
    >
      {rowData.map(({ yearRow, height, monthLabels }) => {
        const yearTotalCount = data
          .filter((activity) => {
            const activityYear = getYear(parseISO(activity.date));
            return activityYear === yearRow.year;
          })
        ...
        return (
          <div key={`year-row-${yearRow.year}`}>
            ...
            {renderYearFooter &&
              renderYearFooter({ year: yearRow.year, totalCount: yearTotalCount })}
          </div>
        );
      })}
    </div>
